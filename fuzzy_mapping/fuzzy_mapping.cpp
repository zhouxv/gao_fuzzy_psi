...
void assign_segments(const std::size_t &elements_size,
  ...
  Rist25519_number value_i = Rist25519_number(prng);
  Rist25519_number rG_value = Rist25519_number(prng);
  Rist25519_number sk_rG_value = sk * rG_value;

  std::vector<Rist25519_number> values_i(elements_size, value_i);

  for (auto i = 0; i < dimension; i++) {

    for (auto j = 0; j < elements_size; j++) {
      // values_i.push_back(value_i);
      //   values_i.push_back(Rist25519_number(prng));
      for (auto k = 0; k < num_interval_per_element_per_dimension; k++) {
        // rG = Rist25519_number(prng);
        // sk_rG = sk * rG;
        vals_candidate_r.push(rG_value);
        vals_candidate_skr.push(sk_rG_value);
      }
    }
...
void get_mask_cipher(const std::size_t &elements_size,
  ...
  PRNG prng(oc::sysRandomSeed());
  Rist25519_number temp_mask = Rist25519_number(prng);
  Rist25519_number temp_mask_inverse = temp_mask.inverse();
  ...
  for (auto i = 0; i < elements_size; i++) {
    ...
    masks_inv.push_back(temp_mask_inverse);
...
/////////////////////////////////////////////////////////////////////////
std::vector<Rist25519_point> fmap_recv_encode_offline(
    ...
    std::stack<Rist25519_number> *recv_vals_candidate_skr, u32 dimension,
    i32 delta, i32 side_length, std::vector<Rist25519_point> &recv_seeds,
    u64 &fmath_recv_size) {
  ...
  fmath_recv_size = recv_fmap_keys.size();
  return recv_fmap_codeWords_net;
}

std::vector<Rist25519_point> fmap_send_encode_offline(
    ...
    std::stack<Rist25519_number> *send_vals_candidate_skr, u32 dimension,
    i32 delta, i32 side_length, std::vector<Rist25519_point> &send_seeds,
  ...
  fmath_sender_size = send_fmap_keys.size();

  return send_fmap_codeWords_net;
}

void fmap_recv_online2(coproto::LocalAsyncSocket *channel,
                       std::vector<std::vector<u64>> *receiver_elements,
  ...
  // std::endl;
  coproto::sync_wait((*channel).send(fmath_recv_size));
...
void fmap_send_online2(coproto::LocalAsyncSocket *channel,
                       std::vector<std::vector<u64>> *sender_elements,
  ...
  coproto::sync_wait((*channel).send(fmath_sender_size));
