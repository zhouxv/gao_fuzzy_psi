...
void assign_segments_fake(const std::size_t &elements_size,
                          std::vector<std::vector<Rist25519_number>> &values,
                          std::stack<Rist25519_number> &vals_candidate_r,
                          std::stack<Rist25519_number> &vals_candidate_skr,
                          const u32 &dimension, const i32 &delta,
  ...
  u64 temp_size =
      dimension * elements_size * num_interval_per_element_per_dimension;
  std::vector<Rist25519_number> temp_0(temp_size);
  std::vector<Rist25519_number> temp_1(temp_size);
  prng.get(temp_0.data(), temp_0.size());
  prng.get(temp_1.data(), temp_1.size());

  values.resize(dimension);
  for (auto i = 0; i < dimension; i++) {
    values[i].resize(elements_size);
    prng.get(values[i].data(), values[i].size());
  }

  for (auto it = temp_0.rbegin(); it != temp_0.rend(); ++it) {
    vals_candidate_r.push(*it);
  }
  for (auto it = temp_1.rbegin(); it != temp_1.rend(); ++it) {
    vals_candidate_skr.push(*it);
  }
...
void get_mask_cipher_fake(const std::size_t &elements_size,
                          std::vector<Rist25519_number> &masks,
                          std::vector<Rist25519_number> &masks_inv,
                          const std::array<Rist25519_point, 2> &pk) {
  PRNG prng(oc::sysRandomSeed());

  masks.resize(elements_size);
  masks_inv.resize(elements_size);
  prng.get(masks.data(), masks.size());
  prng.get(masks_inv.data(), masks_inv.size());
