...
//         std::cout << "test_bp24_lp_low_dim ----------------------------" <<
//         std::endl; PRNG prng(oc::sysRandomSeed());
...
//         if((intersection_size > recv_set_size) | (intersection_size >
//         send_set_size)){
//             printf("intersection_size should not be greater than
//             set_size\n"); return;
//         }
...
//         std::vector<std::vector<u64>> receiver_elements(recv_set_size,
//         std::vector<u64>(dimension, 0)); std::vector<std::vector<u64>>
//         sender_elements(send_set_size, std::vector<u64>(dimension, 0));
...
//         for(u64 i = 0; i < send_set_size; i++){
//             for(u64 j = 0; j < dimension; j++){
//                 sender_elements[i][j] = (prng.get<u64>()) %
//                 ((0xffff'ffff'ffff'ffff) - 3 * delta) + 1.5 * delta;
//             }
//         }
...
//         bp24_ec::receiver_precomp_vals_ec_lp(recv_set_size, fmat_vals,
//         dimension, delta, side_length, p, recv_sk);
...
//         bp24_ec::sender_mask_ec_lp(send_set_size, vec_G_pow_a_H_pow_c, vec_b,
//         vec_G_pow_c, vec_G_pow_a_bj, delta, p, recv_pk[0], recv_pk[1]);
...
// 		std::thread
// thread_fmat_recv(bp24_ec::bp24_lp_low_dim_recv_online, &sockets[0],
//         &receiver_elements,
//         &fmat_vals,
...
// 		std::thread
// thread_fmat_send(bp24_ec::bp24_lp_low_dim_send_online, &sockets[1],
//         &sender_elements,
...
// 		auto recv_bytes_present = sockets[0].bytesSent();
// 		auto send_bytes_present = sockets[1].bytesSent();
//         std::cout << "[bp24_lp] recv sends:  "<< (recv_bytes_present) /
//         1024.0 / 1024 << "MB" << std::endl;
// 		std::cout << "[bp24_lp] send sends:  "<< (send_bytes_present) /
// 1024.0 / 1024 << "MB" << std::endl; 		std::cout << "[bp24_lp] comm
// total:  "<<
// ((recv_bytes_present) + (send_bytes_present)) / 1024.0 / 1024 << "MB" <<
// std::endl;
...
//         if(out_to_file){
//             std::string filename =
//             "test_bp24_lp_low_dim_m_"+std::to_string(send_set_size)
//                                     +"_n_"+std::to_string(recv_set_size)
//                                     +"_d_"+std::to_string(dimension)
//                                     +"_delta_"+std::to_string(delta)
//                                     +"_p_"+std::to_string(p)+".txt";
//             std::ofstream mycout(filename, std::ios::app);
//             mycout << std::endl << "test_bp24_lp_low_dim
//             ----------------------------" << std::endl; mycout <<
//             "recv_set_size: " << recv_set_size << std::endl; mycout <<
//             "send_set_size: " << send_set_size << std::endl; mycout <<
//             "dimension:     " << dimension << std::endl; mycout << "delta: "
//             << delta << std::endl; mycout << "side_length:   " << side_length
//             << std::endl; mycout << "l_p:           " << p << std::endl;
//             mycout << "[bp24_lp] recv sends:  "<< (recv_bytes_present) /
//             1024.0 / 1024 << "MB" << std::endl;
// 		    mycout << "[bp24_lp] send sends:  "<< (send_bytes_present) /
// 1024.0 / 1024 << "MB" << std::endl; 		    mycout << "[bp24_lp] comm
// total:  "<<
// ((recv_bytes_present) + (send_bytes_present)) / 1024.0 / 1024 << "MB" <<
// std::endl;
//             mycout << time << std::endl << std::endl;
...
//         std::cout << "test_bp24_linfty_low_dim ----------------------------"
//         << std::endl; PRNG prng(oc::sysRandomSeed());
...
//         std::vector<std::vector<FM25519_number>> fmat_vals;
//         bp24_ec::receiver_precomp_vals_ec_linfty(recv_set_size, fmat_vals,
//         dimension, delta, side_length, recv_sk);

//         std::vector<FM25519_point> vec_G_pow_a;
//         std::vector<FM25519_number> vec_b;
//         std::vector<FM25519_point> vec_H_pow_a;
//         bp24_ec::sender_mask_ec_linfty(send_set_size, vec_G_pow_a, vec_b,
//         vec_H_pow_a, recv_pk[0], recv_pk[1]); std::cout << "fmat offline
//         done" << std::endl; time.setTimePoint("offline");
// // ///////////////////////////////////////

// 	    auto sockets = coproto::LocalAsyncSocket::makePair();

// 		std::thread
// thread_fmat_recv(bp24_ec::bp24_linfty_low_dim_recv_online, &sockets[0],
//         &receiver_elements,
//         &fmat_vals,
//         dimension, delta, side_length,
//         recv_sk, recv_pk[0]);
...
// 		std::thread
// thread_fmat_send(bp24_ec::bp24_linfty_low_dim_send_online, &sockets[1],
//         &sender_elements,
...
// 		auto recv_bytes_present = sockets[0].bytesSent();
// 		auto send_bytes_present = sockets[1].bytesSent();
//         std::cout << "[bp24_linfty] recv sends:  "<< (recv_bytes_present) /
//         1024.0 / 1024 << "MB" << std::endl;
// 		std::cout << "[bp24_linfty] send sends:  "<<
// (send_bytes_present) / 1024.0 / 1024 << "MB" << std::endl; std::cout <<
// "[bp24_linfty] comm total:  "<< ((recv_bytes_present) + (send_bytes_present))
// / 1024.0 / 1024 << "MB" << std::endl;
...
//         if(out_to_file){
//             std::string filename =
//             "test_bp24_linfty_low_dim_m_"+std::to_string(send_set_size)
//                                     +"_n_"+std::to_string(recv_set_size)
//                                     +"_d_"+std::to_string(dimension)
//                                     +"_delta_"+std::to_string(delta)
//                                     +"_p_infty"+".txt";
//             std::ofstream mycout(filename, std::ios::app);
//             mycout << std::endl << "test_bp24_linfty_low_dim
//             ----------------------------" << std::endl; mycout <<
//             "recv_set_size: " << recv_set_size << std::endl; mycout <<
//             "send_set_size: " << send_set_size << std::endl; mycout <<
//             "dimension:     " << dimension << std::endl; mycout << "delta: "
//             << delta << std::endl; mycout << "side_length:   " << side_length
//             << std::endl; mycout << "distance:      l_infty" << std::endl;
//             mycout << "[bp24_linfty] recv sends:  "<< (recv_bytes_present) /
//             1024.0 / 1024 << "MB" << std::endl;
// 		    mycout << "[bp24_linfty] send sends:  "<<
// (send_bytes_present) / 1024.0 / 1024 << "MB" << std::endl; mycout <<
// "[bp24_linfty] comm total:  "<< ((recv_bytes_present) + (send_bytes_present))
// / 1024.0 / 1024 << "MB" << std::endl;
//             mycout << time << std::endl << std::endl;
//             mycout.close();
...
//         std::cout << "test_bp24_lp_high_dim ----------------------------" <<
//         std::endl; PRNG prng(oc::sysRandomSeed());
...
//         double time = (N * L * (2 * delta + 1) * dimension * 40 + M * L * T *
//         dimension) * 10 * 0.001; double comm = N * L * (2 * delta + 1) *
//         dimension * EC_CIPHER_SIZE_IN_BLOCK * 16 / 1024.0 / 1024;

//         std::cout << "rho          : " << rho << std::endl;

// 		std::cout << "[bp24_lp_high] comm lower bound:  " << comm <<
// "MB" << std::endl;
//         std::cout << "[bp24_lp_high] time lower bound:  " << time << "ms" <<
//         std::endl << std::endl;
...
//         if(out_to_file){
//             std::string filename =
//             "test_bp24_lp_high_dim_m_"+std::to_string(send_set_size)
//                                     +"_n_"+std::to_string(recv_set_size)
//                                     +"_d_"+std::to_string(dimension)
//                                     +"_delta_"+std::to_string(delta)
//                                     +"_p_"+std::to_string(p)+".txt";
//             std::ofstream mycout(filename, std::ios::app);
//             mycout << std::endl << "test_bp24_lp_high_dim
//             ----------------------------" << std::endl; mycout <<
//             "recv_set_size: " << recv_set_size << std::endl; mycout <<
//             "send_set_size: " << send_set_size << std::endl; mycout <<
//             "dimension:     " << dimension << std::endl; mycout << "delta: "
//             << delta << std::endl; mycout << "side_length:   " << side_length
//             << std::endl; mycout << "l_p:           " << p << std::endl;
// 		    mycout << "[bp24_lp_high] comm lower bound:  " << comm <<
// "MB" << std::endl;
//             mycout << "[bp24_lp_high] time lower bound:  " << time << "ms" <<
//             std::endl << std::endl; mycout.close();
//         }
...
//         std::cout << "test_bp24_linfty_high_dim ----------------------------"
//         << std::endl; PRNG prng(oc::sysRandomSeed());
...
//         std::vector<std::vector<u64>> receiver_elements(recv_set_size,
//         std::vector<u64>(dimension, 0)); std::vector<u64>
//         separate_dims(recv_set_size);
...
//         bp24_high_dim::receiver_precomp_vals_ec_linfty(recv_set_size,
//         vals_candidate, dimension, delta, recv_sk);

//         std::vector<FM25519_point> vec_G_pow_a;
//         std::vector<FM25519_number> vec_b;
//         std::vector<FM25519_point> vec_H_pow_a;
//         bp24_high_dim::sender_mask_ec_linfty(send_set_size, vec_G_pow_a,
//         vec_b, vec_H_pow_a, recv_pk[0], recv_pk[1]);
...
// 		std::thread
// thread_fmat_recv(bp24_high_dim::bp24_linfty_high_dim_recv_online,
// &sockets[0],
...
// 		std::thread
// thread_fmat_send(bp24_high_dim::bp24_linfty_high_dim_send_online,
// &sockets[1],
//         &sender_elements,
//         &vec_G_pow_a, &vec_b, &vec_H_pow_a,
//         dimension, delta);
...
// 		auto recv_bytes_present = sockets[0].bytesSent();
// 		auto send_bytes_present = sockets[1].bytesSent();
//         std::cout << "[bp24_linfty_high] recv sends:  "<<
//         (recv_bytes_present) / 1024.0 / 1024 << "MB" << std::endl;
// 		std::cout << "[bp24_linfty_high] send sends:  "<<
// (send_bytes_present) / 1024.0 / 1024 << "MB" << std::endl; std::cout <<
// "[bp24_linfty_high] comm total:  "<< ((recv_bytes_present) +
// (send_bytes_present)) / 1024.0 / 1024 << "MB" << std::endl;
...
//         if(out_to_file){
//             std::string filename =
//             "test_bp24_linfty_high_dim_m_"+std::to_string(send_set_size)
//                                     +"_n_"+std::to_string(recv_set_size)
//                                     +"_d_"+std::to_string(dimension)
//                                     +"_delta_"+std::to_string(delta)
//                                     +"_p_infty"+".txt";
//             std::ofstream mycout(filename, std::ios::app);
//             mycout << std::endl << "test_bp24_linfty_high_dim
//             ----------------------------" << std::endl; mycout <<
//             "recv_set_size: " << recv_set_size << std::endl; mycout <<
//             "send_set_size: " << send_set_size << std::endl; mycout <<
//             "dimension:     " << dimension << std::endl; mycout << "delta: "
//             << delta << std::endl; mycout << "side_length:   " << side_length
//             << std::endl; mycout << "distance:      l_infty" << std::endl;
//             mycout << "[bp24_linfty_high] recv sends:  "<<
//             (recv_bytes_present) / 1024.0 / 1024 << "MB" << std::endl;
// 		    mycout << "[bp24_linfty_high] send sends:  "<<
// (send_bytes_present) / 1024.0 / 1024 << "MB" << std::endl; mycout <<
// "[bp24_linfty_high] comm total:  "<< ((recv_bytes_present) +
// (send_bytes_present)) / 1024.0 / 1024 << "MB" << std::endl;
//             mycout << time << std::endl << std::endl;
