...
// // without setup: input masks and vec_masked_distance should be two empty
// vectors. void sender_q_to_mask_distance_paillier_lp(const
...
void pad_send_prefixes_k(
    ...
    std::vector<DH25519_point> &send_prefixes_k_net, const u64 &max_prefix_num);

// todo: when iter - block(pow(delta, p)) or iter + block(pow(delta, p)) is not
// in [0, 2^32 - 1] input vec_prefixes should be an empty vector.
...
std::vector<block> ec_cipher_to_block_vector(const FM25519_point &G,
                                             const FM25519_point &sG_M);

void block_vector_to_ec_cipher(const std::vector<block> &cipher_block,
                               FM25519_point &G, FM25519_point &sG_M);
...
void receiver_value_ec_lp(const std::size_t &elements_size,
                          ...
                          const u32 &dimension, const i32 &delta, const u32 &p,
                          const FM25519_number &sk, const FM25519_point &G_fm);

// setup: input vec_G_pow_a, vec_b, vec_G_pow_c, vec_H_pow_c, and vec_G_pow_a_bj
...
void sender_mask_ec_lp(const std::size_t &elements_size,
                       ...
                       const i32 &delta, const u32 &p, const FM25519_point &G,
                       const FM25519_point &H);

void receiver_w_to_key_ec_lp(const std::vector<std::vector<u64>> &elements,
                             const std::vector<Rist25519_point> &vec_keyw,
                             std::vector<block> &keys, const u32 &dimension,
                             const i32 &delta);

void sender_q_to_masked_distance_ec_lp(
    const std::vector<std::vector<u64>> &elements,
    const std::vector<Rist25519_point> &vec_kq,
    ...
    std::vector<FM25519_point> &vec_F_star,
    std::vector<FM25519_point> &vec_H_star, const u32 &dimension,
    const i32 &okvs_n);

void receiver_result(
    const std::vector<FM25519_point> &vec_F_star,
    const std::vector<FM25519_point> &vec_H_star,
    const std::vector<std::vector<FM25519_point>> &vec_G_pow_a_bj,
    std::vector<bool> &result, const FM25519_number &sk);
...
void fmat_ec_recv_online(coproto::LocalAsyncSocket *channel,
                         std::vector<std::vector<u64>> *receiver_elements,
                         std::vector<FM25519_point> *recv_vec_dhkk_seedsum,
                         std::vector<std::vector<Rist25519_number>> *fmat_vals,
                         u64 dimension, u64 delta, Rist25519_number recv_sk);
...
void fmat_ec_send_online(coproto::LocalAsyncSocket *channel,
                         ...
                         std::vector<FM25519_point> *vec_G_pow_a,
                         std::vector<FM25519_number> *vec_b,
                         std::vector<FM25519_point> *vec_H_pow_a, u64 dimension,
                         u64 delta);
