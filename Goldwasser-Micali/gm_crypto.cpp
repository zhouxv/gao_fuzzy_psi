...
mpz_t *enc_bit(mpz_t *cyphertext, unsigned short plaintext, pubkey_t *pbkey)
{
        if(!cyphertext)
        {
                cyphertext = (mpz_t *)malloc(sizeof(mpz_t));
                mpz_init(*cyphertext);
        }
...
mpz_t **enc_bitstream(mpz_t **cyphertext, unsigned short *plaintext, unsigned short msg_size, pubkey_t *pbkey)
{
        if(!cyphertext)
        {
                cyphertext = (mpz_t **)malloc(msg_size * sizeof(mpz_t*));
                ...
                {
                        *(cyphertext + i) = (mpz_t *)malloc(sizeof(mpz_t));
...
{
        if(!plaintext)
                plaintext = (unsigned short *)malloc(sizeof(unsigned short));
...
unsigned short **dec_bitstream(unsigned short **plaintext, mpz_t **cyphertext, unsigned short msg_size, privkey_t *prkey)
{
        if(!plaintext)
        {
                plaintext = (unsigned short **)malloc(msg_size * sizeof(unsigned short*));
                for(int i=0; i<msg_size; i++)
                        *(plaintext+i) = (unsigned short *)malloc(sizeof(unsigned short));                
        }
...
mpz_t *xor_on_cipher(mpz_t *cyphertext, mpz_t *cyphertext_a, mpz_t *cyphertext_b, pubkey_t *pbkey){
        ...
        mpz_t temp;
        mpz_init(temp);
        mpz_mul(temp, *cyphertext_a, *cyphertext_b);
        mpz_mod(*cyphertext, temp, pbkey->N);
        mpz_clear(temp);
...
mpz_t **xor_on_cipher_bitstream(mpz_t **cyphertext, mpz_t **cyphertext_a, mpz_t **cyphertext_b, unsigned short msg_size, pubkey_t *pbkey)
{
        ...
        for(int i=0; i<msg_size; i++)
                xor_on_cipher(*(cyphertext+i), *(cyphertext_a+i),*(cyphertext_b+i), pbkey);

        return cyphertext;
}

void gm_cipher_to_block_vector(const  mpz_t *cyphertext, oc::block* block_vec){
        mpz_export(&*block_vec, NULL, -1, sizeof(oc::block), 0, 0, *cyphertext);
}


void block_vector_to_gm_cipher(const oc::block* block_vec, mpz_t *cyphertext){
        // oc::u8 temp_bytes[POINT_LENGTH_IN_BYTE];
        // memcpy(temp_bytes, block_vec[0].data(), sizeof(oc::block));
        // memcpy(temp_bytes + sizeof(oc::block), block_vec[1].data(), sizeof(oc::block));
        //cyphertext.fromBytes(temp_bytes);
        mpz_import(*cyphertext, GM_CIPHER_LENGTH_BLOCKS, -1, sizeof(oc::block), 0, 0, &*block_vec);
        return;
}
